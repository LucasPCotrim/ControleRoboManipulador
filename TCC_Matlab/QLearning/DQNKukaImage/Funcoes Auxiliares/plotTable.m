function plotTable()
    ...
    % Ground
    cube_plot([-0.2 -3 -0.05],6,6,0.05,[0 0 1]);
    
    % Obstacle
    x_obs_plot = x_obs - 0.5*obstacle_width;
    y_obs_plot = y_obs - 0.5*obstacle_width;
    z_obs_plot = z_obs - 0.5*obstacle_width;
    cube_plot([x_obs_plot y_obs_plot z_obs_plot], obstacle_width, obstacle_width, obstacle_width, 'red')
    
    % Set-Point
    x_sp_plot = x_sp - 0.5*setpoint_width;
    y_sp_plot = y_sp - 0.5*setpoint_width;
    z_sp_plot = z_sp - 0.5*setpoint_width;
    cube_plot([x_sp_plot y_sp_plot z_sp_plot], setpoint_width, setpoint_width, setpoint_width, 'green')
end
